	...
	'ridicule': {
		'function': async (msg, [command, on]) => {
			...
			if (command === 'turn') {
				if (on) {
					if (on === 'on') {
						database.set((msg.guild || msg.author).id, true, 'settings.ridicule.on');
						await msg.channel.send(getString('ridicule_on'));
					} else if (on === 'off') {
						database.set((msg.guild || msg.author).id, false, 'settings.ridicule.on');
						await msg.channel.send(getString('ridicule_off'));
					} else {
						await msg.channel.send(getString('ridicule_bad'));
					}
				} else {
					const on = database.get((msg.guild || msg.author).id, 'settings.ridicule.on');
				...
				if (on) {
					...
					if (!isNaN(chance) && chance > 0) {
						database.set((msg.guild || msg.author).id, chance, 'settings.ridicule.chance');
					...
					const chance = database.get((msg.guild || msg.author).id, 'settings.ridicule.chance');
