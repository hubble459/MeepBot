...
async function alias(msg, msgArgs) {
	...
	if (msgArgs.length > 1) {
		...
		} else if (newCommand.startsWith(database.get((msg.guild || msg.author).id, 'settings.prefix'))) {
			await msg.channel.send(getString('alias_prefix'));
		} else {
			let command = msgArgs[1];
			if (!commands[command] && !database.has((msg.guild || msg.author).id, `aliases.${command}`) && command !== 'help') {
				return msg.channel.send(getString('alias_not_command').format(command));
			}
			const args = msgArgs.slice(2);

			const existed = database.has((msg.guild || msg.author).id, `aliases.${newCommand}`);
			database.set((msg.guild || msg.author).id, {
				command,
				args
			}, `aliases.${newCommand}`);
...
const aliasrm = {
	'function': async (msg, [alias]) => {
		const getString = getGetString((msg.guild || msg.author).id);
		if (alias) {
			const exists = database.has((msg.guild || msg.author).id, `aliases.${alias}`);
			if (exists) {
				database.delete((msg.guild || msg.author).id, `aliases.${alias}`);
...
const aliases = {
	'function': async (msg, [arg1]) => {
		...
		if (arg1 === 'reset') {
			database.set((msg.guild || msg.author).id, defaultAliases, 'aliases');
			await msg.channel.send(getString('aliases_reset'));
		} else {
			const aliases = database.get((msg.guild || msg.author).id, 'aliases');
			const values = [];
