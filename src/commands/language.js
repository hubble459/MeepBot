...
async function language(msg, [lang]) {
	if (!lang) {
		...
		const locale = database.get((msg.guild || msg.author).id, 'settings.language');
		if ('tsundere'.startsWith(locale) && (permissions & 32768) === 32768) {
			await msg.channel.send(tsundereAttachment);
		} else {
			const string = getString((msg.guild || msg.author).id, 'language_current').format(locale);
			await msg.channel.send(string);
		}
	} else {
		lang = find(lang);
		if (lang) {
			database.set((msg.guild || msg.author).id, lang, 'settings.language');
			const string = getString((msg.guild || msg.author).id, 'language_changed');
			await msg.channel.send(string.format(lang));
		} else {
			const string = getString((msg.guild || msg.author).id, 'language_not_found').format(lang);
			await msg.channel.send(string.format(lang));
		}
	}
}
